...
class KerasGenerator(tf.keras.utils.Sequence):
    def __init__(self, data_generator, img_width,
                 img_height,
                 batch_size=32,
                 shuffle=True,
                 transformations=[],
    ...
    def on_epoch_end(self):
        ...
        if self.shuffle:
            objects_to_shuffle = [self.data_generator.dataset_indices]
            if not (self.data_generator.filenames is None):
                objects_to_shuffle.append(self.data_generator.filenames)
            if not (self.data_generator.labels is None):
                objects_to_shuffle.append(self.data_generator.labels)
            if not (self.data_generator.image_ids is None):
                objects_to_shuffle.append(self.data_generator.image_ids)
            if not (self.data_generator.eval_neutral is None):
                objects_to_shuffle.append(self.data_generator.eval_neutral)
    ...
    def __getitem__(self, index):
        if self.data_generator.dataset_size == 0:
        ...
        if self.data_generator.labels is None:
            if any([ret in self.returns for ret in ['original_labels', 'processed_labels', 'encoded_labels', 'matched_anchors', 'evaluation-neutral']]):
            ...
            if any([ret in self.returns for ret in ['encoded_labels', 'matched_anchors']]):
        ...
        elif not isinstance(self.label_encoder, SSDInputEncoder):
        ...
        if self.degenerate_box_handling == 'remove':
            box_filter = BoxFilter(check_overlap=False,
                                   ...
                                   labels_format=self.data_generator.labels_format)
        ...
        batch_X, batch_y = [], []
        index %= len(self.data_generator.dataset_indices) // self.batch_size
        ...
        #    files from disk.
        batch_indices = self.data_generator.dataset_indices[index * self.batch_size : (index + 1) * self.batch_size]
        if not (self.data_generator.images is None):
            for i in batch_indices:
                batch_X.append(self.data_generator.images[i])
            if not (self.data_generator.filenames is None):
                batch_filenames = self.data_generator.filenames[index * self.batch_size : (index + 1) * self.batch_size]
            else:
                batch_filenames = None
        elif not (self.data_generator.hdf5_dataset is None):
            for i in batch_indices:
                batch_X.append(self.data_generator.hdf5_dataset['images'][i].reshape(self.data_generator.hdf5_dataset['image_shapes'][i]))
            if not (self.data_generator.filenames is None):
                batch_filenames = self.data_generator.filenames[index * self.batch_size : (index + 1) * self.batch_size]
            else:
                batch_filenames = None
        ...
        if not (self.data_generator.labels is None):
            batch_y = deepcopy(self.data_generator.labels[index * self.batch_size : (index + 1) * self.batch_size])
        else:
            batch_y = None

        if not (self.data_generator.eval_neutral is None):
            batch_eval_neutral = self.data_generator.eval_neutral[index * self.batch_size : (index + 1) * self.batch_size]
        else:
            batch_eval_neutral = None

        # Get the image IDs for this batch (if there are any).
        if not (self.data_generator.image_ids is None):
            batch_image_ids = self.data_generator.image_ids[index * self.batch_size : (index + 1) * self.batch_size]
        else:
        ...
        for i in range(len(batch_X)):
            ...
            # print("Maybe generate")
            if self.transformations:
                inverse_transforms = []
                for transform in self.transformations:
                    # print("Generate:)")
                    if not (self.data_generator.labels is None):
                        if ('inverse_transform' in self.returns) and ('return_inverter' in inspect.signature(transform).parameters):
            ...
            if not (self.data_generator.labels is None):
                xmin = self.data_generator.labels_format['xmin']
                ymin = self.data_generator.labels_format['ymin']
                xmax = self.data_generator.labels_format['xmax']
                ymax = self.data_generator.labels_format['ymax']
        ...
        if batch_items_to_remove:
            for j in sorted(batch_items_to_remove, reverse=True):
                ...
                if not (self.data_generator.labels is None): batch_y.pop(j)
                if not (self.data_generator.image_ids is None): batch_image_ids.pop(j)
                if not (self.data_generator.eval_neutral is None): batch_eval_neutral.pop(j)
        ...
        if not (self.label_encoder is None or self.data_generator.labels is None):
        ...
        ret = []
        if 'processed_images' in self.returns: ret.append(batch_X / 127 - 1)
        ...
        if 'original_images' in self.returns: ret.append(np.array(batch_original_images) / 127 - 1)
        if 'original_labels' in self.returns: ret.append(batch_original_labels)
        return tuple(ret)
    
    def __len__(self):
        return self.data_generator.dataset_size
...
class DataGenerator:
    ...
    def parse_one_json(self,
        ...
        annos_by_images = {}
        names = list({annotation['category_id'] for annotation in annotations["annotations"]})
        for annotation in annotations["annotations"]:
            if annotation["image_id"] not in annos_by_images.keys():
                annos_by_images[annotation["image_id"]] = []
            val = names.index(annotation['category_id'])
            xmin, ymin, width, height = annotation["bbox"]
            annos_by_images[annotation["image_id"]].append((val + 1, xmin, ymin, xmin + width, ymin + height))

        for cat in annotations["annotations"]:
            if names.index(cat['category_id']) + 1 not in self.cats_to_names.keys():
                self.cats_to_names[names.index(cat['category_id']) + 1] = annotations["categories"][cat['category_id']]["name"]
                self.classes_to_names.append(annotations["categories"][cat['category_id']]["name"])
                self.cats_to_classes[names.index(cat['category_id']) + 1] = names.index(cat['category_id']) + 1
                self.classes_to_cats[names.index(cat['category_id']) + 1] = names.index(cat['category_id']) + 1
                i += 1

            # Loop over all images in this dataset.
        for image_id, values in annos_by_images.items():
            img_filename = annotations["images"][image_id]["file_name"]
            self.filenames.append(os.path.join(images_dir, img_filename))
            ...
            if ground_truth_available:
                ...
                for bbox in values:
                    ...
                    item_dict = {'image_name': img_filename[:img_filename.find('.')],
                                 'image_id': image_id,
                                 'class_id': class_id,
                                 'xmin': xmin,
                                 'ymin': ymin,
    ...
    def parse_json(self,
        ...
        '''
        self.images_dir = images_dir
        self.annotations_filenames = [f"{annotations_dir}/{file}" for file in annotations_filenames]
        self.include_classes = include_classes
        # Erase data that might have been parsed before.
        self.filenames = []
        self.image_ids = []
        ...
        for annotation_filename in annotations_filenames:
            with open(f"{annotations_dir}/{annotation_filename}", 'r') as f:
                annotations = json.load(f)
            for key in annotations.keys():
                if key.find('item') != -1:
                    cat = annotations[key]
                    if cat['category_id'] not in self.cats_to_names.keys():
                        self.cats_to_names[cat['category_id']] = cat['category_name']
                        self.classes_to_names.append(cat['category_name'])
                        self.cats_to_classes[cat['category_id']] = i
                        self.classes_to_cats[i] = cat['category_id']
                        i += 1

            # Loop over all images in this dataset.
        for i, annotation_filename in enumerate(annotations_filenames):
        # for img in it:
            self.filenames.append(os.path.join(images_dir, f"{annotation_filename[:annotation_filename.find('.')]}.jpg"))
            self.image_ids.append(i)

            if ground_truth_available:
                # Get all annotations for this image.
                with open(f"{annotations_dir}/{annotation_filename}", 'r') as reader:
                    annotations = json.load(reader)

                boxes = []
                for key in annotations.keys():
                    if key.find("item") != -1:
                        cat_id = annotations[key]['category_id']
                        ...
                        class_id = self.cats_to_classes[cat_id]
                        xmin = annotations[key]['bounding_box'][0]
                        ymin = annotations[key]['bounding_box'][1]
                        xmax = annotations[key]['bounding_box'][2]
                        ymax = annotations[key]['bounding_box'][3]
                        # Compute `xmax` and `ymax`.
                        # xmax = xmin + width
                        # ymax = ymin + height
                        item_dict = {'image_name': annotation_filename[:annotation_filename.find('.')],
                                     'image_id': i,
                                     'class_id': class_id,
                                     'xmin': xmin,
                                     'ymin': ymin,
