...
class CropPad:
    ...
    def __call__(self, image, labels=None, return_inverter=False):
        ...
        if image.ndim == 3:
            canvas = np.zeros(shape=(patch_height, patch_width, 3), dtype=np.uint8)
            canvas[:, :] = self.background
        elif image.ndim == 2:
            canvas = np.zeros(shape=(self.patch_height, self.patch_width), dtype=np.uint8)
            canvas[:, :] = self.background[0]
        assert canvas.shape == (patch_height, patch_width, 3), \
        f"I don't know why, but {canvas.shape} not equal {(self.patch_height, self.patch_width, 3)}"
        # Perform the crop.
        if patch_ymin < 0 and patch_xmin < 0: # Pad the image at the top and on the left.
            image_crop_height = min(img_height, patch_height + patch_ymin)  # The number of pixels of the image that will end up on the canvas in the vertical direction.
            image_crop_width = min(img_width, patch_width + patch_xmin) # The number of pixels of the image that will end up on the canvas in the horizontal direction.
            ...
            image_crop_height = min(img_height, patch_height + patch_ymin)  # The number of pixels of the image that will end up on the canvas in the vertical direction.
            image_crop_width = min(patch_width, img_width - patch_xmin) # The number of pixels of the image that will end up on the canvas in the horizontal direction.
            ...
            image_crop_height = min(patch_height, img_height - patch_ymin) # The number of pixels of the image that will end up on the canvas in the vertical direction.
            image_crop_width = min(img_width, patch_width + patch_xmin) # The number of pixels of the image that will end up on the canvas in the horizontal direction.
...
class Crop:
    ...
    def __call__(self, image, labels=None, return_inverter=False):
        ...
        crop.patch_height = img_height - self.crop_top - self.crop_bottom
        crop.patch_width = img_width - self.crop_left - self.crop_right
        crop.labels_format = self.labels_format
...
class Pad:
    ...
    def __call__(self, image, labels=None, return_inverter=False):
        ...
        pad = CropPad(patch_ymin=-self.pad_top,
                       ...
                       labels_format=self.labels_format)
        pad.patch_height = img_height + self.pad_top + self.pad_bottom
        pad.patch_width = img_width + self.pad_left + self.pad_right
        pad.labels_format = self.labels_format
...
class RandomPatch:
    ...
    def __call__(self, image, labels=None, return_inverter=False):
        sample_patch = CropPad(patch_ymin=None,
                                patch_xmin=None,
                                patch_height=None,
                                patch_width=None,
                                clip_boxes=self.clip_boxes,
        ...
        if p >= (1.0-self.prob):
            ...
            img_height, img_width = image.shape[:2]
            patch_coord_generator.img_height = img_height
            patch_coord_generator.img_width = img_width
            ...
            sample_patch.labels_format = self.labels_format
            ...
            for _ in range(max(1, self.n_trials_max)):
                ...
                sample_patch.patch_ymin = patch_ymin
                sample_patch.patch_xmin = patch_xmin
                sample_patch.patch_height = patch_height
                sample_patch.patch_width = patch_width
        ...
        while True: # Keep going until we either find a valid patch or return the original image.
            ...
            if p >= (1.0-self.prob):
                ...
                for _ in range(max(1, self.n_trials_max)):
                    ...
                    # Generate patch coordinates.
                    patch_ymin, patch_xmin, patch_height, patch_width = patch_coord_generator()
                    ...
                    if not (patch_coord_generator.min_aspect_ratio <= aspect_ratio <= patch_coord_generator.max_aspect_ratio):
                        continue
...
class RandomMaxCropFixedAR:
    ...
    def __call__(self, image, labels=None, return_inverter=False):
        ...
        random_patch = RandomPatch(patch_coord_generator=patch_coord_generator, # Just a dummy object
                                        box_filter=self.box_filter,
